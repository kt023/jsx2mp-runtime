/*!
 * jsx2mp-runtime.quickapp.js v0.4.17
...
var Component = function () {
  ...
  _proto._updateChildProps = function _updateChildProps(tagId, props) {
    var chlidInstanceId = this.props["tagId"] + "-" + tagId;
          ...
          id = _ref.id;
      {
        nextTick$1(function () {
          Object.assign(method, {
            current: _this2._internal.$element(name)
          });
        });
      }
    });
    ...
    if (this._internal.$spliceData) {
      ...
      setDataTask.push(function (callback) {
        for (var _key2 in data) {
          if (_key2 === '$ready') {
            $ready = _this5._internal.$ready !== data.ready;
          }
          if (!(_key2 in _this5._internal)) {
            _this5._internal.$set(_key2, data[_key2]);
          } else if (isDifferentData(_this5._internal, data)) {
            _this5._internal[_key2] = data[_key2];
          }
        }
        nextTick$1(callback);
      });
      ...
      hide = _ref.hide;
  {
    return {
      onInit: function onInit() {
        ...
        var system = require('@system.device');
        system.getInfo({
      ...
      onShow: function onShow() {
        var routes = this.$app.$def.globalRoutes || {};
        setRoutes(routes);
...
function getId (type, internal) {
  switch (type) {
    case 'tag':
      ...
      if (internal["_attrs"]["tagId"]) {
        return internal["_attrs"]["tagId"];
      }
      return "t_" + _customId++;
    default:
      return "d_" + _customId++;
...
function registerEventsInConfig(Klass, events) {
  ...
  events.forEach(function (eventName) {
    var eventBindTarget = getNativeEventBindTarget(Klass);
    eventBindTarget[eventName] = function () {
      ...
      if (Klass.prototype.__nativeEventMap[eventName]) {
        Klass.prototype.__nativeEventMap[eventName].forEach(function (callback) {
...
function removeNativeEventListener(eventName, callback) {
  var pageInstance = getPageInstance();
  if (pageInstance.__proto__.__nativeEventMap[eventName]) {
    pageInstance.__proto__.__nativeEventMap[eventName] = pageInstance.__proto__.__nativeEventMap[eventName].filter(function (fn) {
      return fn !== callback;
    });
  }
}
  ...
  return getNativeComponentLifecycle({
    ...
    didUpdate: function didUpdate(prevProps, nextProps) {
      if (this.instance && /^t_\d+$/.test(this.instance.instanceId) && this.data.$ready && !shallowEqual(prevProps, nextProps)) {
        this.instance.nextProps = Object.assign({}, this.instance.props, this["_attrs"]);
  ...
  if (Array.isArray(events)) {
    events.forEach(function (eventName) {
      methods[eventName] = function () {
        ...
        if ( !this.instance && this._parent.instance) {
        ...
        if (event) {
          {
            ...
            event._target = _extends({}, event._currentTarget || event._target);
...
function runApp() {
  ...
  var appOptions = {
    ...
    onShareAppMessage: function onShareAppMessage(shareOptions) {
  ...
  };
  {
    return Object.assign(appOptions, {
      onCreate: function onCreate(launchOptions) {
        var _onCreate = staticConfig.onCreate;
        _onCreate && _onCreate();
        var launchQueue = cycles.create;
      ...
      globalRoutes: __updateRouterMap(staticConfig),
      login: staticConfig.login
    });
  }
}
...
function generateBaseOptions(internal, defaultProps) {
  ...
  var props = Object.assign.apply(Object, [{}, defaultProps, internal["_attrs"], {
    "tagId": instanceId,
    $slots: _extends({}, internal["_attrs"].$slots, internal["_attrs"].$scopedSlots)
